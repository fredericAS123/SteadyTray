...
def main():
    parser = argparse.ArgumentParser(description="Stage 1 locomotion sim2sim in MuJoCo")
    parser.add_argument("--policy", type=str, default="model/policy.jit", help="Path to Stage 1 JIT policy")
    parser.add_argument(
        "--config",
        type=str,
        default="deploy/configs/g1_stage1_locomotion.yaml",
        help="Path to Stage 1 MuJoCo config",
    )
